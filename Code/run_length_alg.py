# 4) Create a script which contains a function which applies a run-lenght encoding from a series of bytes
# given.
...
def encode_message(message):
    ...
    while (i <= len(message)-1):
        ...
        while (j < len(message)-1):
            if (message[j] == message[j + 1]): #we look if we have the same letters
                count = count + 1
                j = j + 1
            else:
                break
